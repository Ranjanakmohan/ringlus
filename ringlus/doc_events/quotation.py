...
def get_opportunity(doctype,target,e,r,t,filter):
    ...
    query = """ SELECT O.name, O.party_name FROM `tabOpportunity` as O INNER JOIN `tabBudget BOM` as BB ON BB.opportunity = O.name  WHERE BB.status='To Quotation' and O.status='Open' and BB.docstatus = 1 {0} GROUP BY O.name """.format(condition)
...
def get_updated_costs(budget_boms):
    ...
    items = []
    for i in data:
        bb = frappe.db.sql(""" SELECT BBFD.item_code, BB.total_raw_material_cost, BB.total_operation_cost, BB.total_additional_operational_cost
                              FROM `tabBudget BOM` BB 
                              INNER JOIN `tabBudget BOM Details` BBFD ON BBFD.parent=BB.name and BBFD.parentfield='fg_bom_details' 
                              WHERE BB.name=%s """, i['budget_bom'], as_dict=1)
        print("===================================")
        print(bb)
        items.append(bb)
    return items